...
### 2026-06-01 — Authored two Advanced challenges (Eval & Red Teaming + Action Tools)

**Scope:** Wrote student `README.md` + coach `solution.md` + `validate.py` for both, using RESTRUCTURE-SPEC §3.2 banner and §3 STEP template (Goal → Tasks → Success Criteria → Checkpoint).

**Evaluation & Red Teaming** (`challenges/advanced-evaluation-redteam/`):
...
- `validate.py --step {1..4}` / `--all` — all offline (dry-run/custom-only) so coaches don't burn quota. **All checkpoints PASS.**

**Action Tools** (`challenges/advanced-action-tools/`):
...
**QA notes / patterns:**
- `create_file` refuses to overwrite; to replace harvested v1 content (old challenge-05 README/solution), truncate with `: > file` (backups to /tmp) then author. This is full-file rewrite, not hand-editing via terminal.
- Leakage check: grep student READMEs for `coach|answer key|reference completion|def run_with_approval` — only benign "ask your coach" phrasing remained; no reference impls leaked.
...
**Requested by Marco (coordinator). Three linear QA tasks. No git commit; docs/ prose untouched (Linus owns mirror); coach answers stay only in solution.md.**

**Task 1 — ENV VAR RECONCILE (challenges/ only).** Grepped all of `challenges/` for env names NOT in the `.env.sample` contract and reconciled to the authoritative names. Renames applied (perl ordered-substitution for .md to avoid `MODEL_DEPLOYMENT_NAME` substring collisions; edit-tool for .py):
...
- `challenges/advanced-action-tools/validate.py` (PATCHED) — added `--dry-run`: Step1 checks provided `app.py` present (REST health skipped), Step4 checks `mcp_server.py` present (round-trip skipped); Steps 2/3 already static.
...
**Task 3 — Prompt Flow grep sweep.** `.devcontainer/` exists and is in scope. Sweep of `challenges/ resources/ requirements.txt .devcontainer/`:
- **challenges/ — CLEAN of accidental refs.** All 4 hits are *deliberate guardrails*: deploy-hosted README:36 "No Prompt Flow here" box + solution.md:6/8/115 "rewritten away from Prompt Flow" coach context. KEEP.
- **requirements.txt:18** — guardrail comment "replaces deprecated Prompt Flow". KEEP.
- **.devcontainer/** — no hits.
- **resources/QA-REPORT.md:39** — 1 *stale historical* reference to the old `challenge-03-prompt-flow/` structure + `flow.dag.yaml` artifacts. Left AS-IS: it is a point-in-time QA record, not curriculum; rewriting would falsify the report. **Flagged to coordinator.**
- **docs/ (NOTE for Linus, NOT fixed):** 8 hits — `docs/coach-hub.md:61,72`; `docs/challenges/challenge-04-coach.md:52,59`; `docs/challenges/challenge-04.md:133,135,167`; `docs/challenges/challenge-06.md:64,66,227`. These are the old Prompt Flow curriculum in the docs mirror; Linus owns reconciliation.
...
**Advisory (printed, NOT gating) — the deliberate reconciliation:** Marco's task asked to also detect KB-reuse + Action-Tools-approval-loop. But Danny's contract AND the README mark criterion 4 (reuse) as **coach-confirmed/live, NOT in validate.py**. Constraint: "align to the README (authoritative)." **Resolution:** I detect both by import/reference (`KB_RE`: AZURE_SEARCH/AzureAISearch/university-faq/foundry_iq/…; `APPROVAL_RE`: RequiredMcpToolCall/ToolApproval/SubmitToolApprovalAction/ACTION_MCP_URL/northfield_actions/McpTool) and print them under an `— advisory (coach-confirmed live, not gating) —` block with ✅/➖ marks — so they inform but never change exit code. Noted in the decision drop.

**`--list`** echoes the three buckets: AUTO-GRADED (steps 1–3), ADVISORY (reuse hints), and COACH-JUDGED/MANUAL (both topologies shown, DevUI visual, OTel multi-agent span tree, 2-min demo, hosted background run) — so coaches know exactly what's machine-checked vs eyeballed.
...
PLAN-V3 is now **implemented** (staged, not committed). My piece: `challenges/capstone-multi-agent/validate.py`
