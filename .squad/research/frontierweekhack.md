...
> Description: *"Hands-on Microsoft Foundry hackathon labs to build, monitor, evaluate, and deploy agentic AI workflows."*
...
FrontierWeekHack is the official **Microsoft Cloud & AI Frontier Week** hands-on lab. It is a **code-first** (Python SDK) hackathon that walks a participant through the **full lifecycle of a production agentic system**: build → monitor → evaluate → orchestrate/deploy. It deliberately mixes IDE work with the **Microsoft Foundry portal** (`ai.azure.com/nextgen`) so learners experience both the SDK and the no-code surfaces.
...
## 2. Lab / Challenge Inventory

The repo ships **3 parallel scenario tracks**, each with the **same 5-challenge spine**. Pick one track; all teach the same Foundry features.
...
### Per-track challenge spine (identical across all 3)

| # | Challenge | Topics taught | Foundry features used | Est. time |
|---|-----------|---------------|----------------------|-----------|
...
**Provisioning is a single Bash script per track**, not azd/bicep/Terraform. See [`factory/challenge-0-setup/deploy.sh`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-0-setup/deploy.sh).

What `deploy.sh` does:
1. `SUFFIX=$(openssl rand -hex 4)` → unique resource names; RG `foundry-hackathon-rg-<suffix>`, region `swedencentral`.
...
- **No AI Search / RAG challenge.** File Search + knowledge bases + vector stores are *described* conceptually in [`challenge-1-build/README.md`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-1-build/README.md) but **never built**. There is no AI Search index, no document upload, no grounding lab. **Big opportunity** for our expanded curriculum — a real RAG/knowledge-base challenge.
...
|-------|-----------|-----------------|
| **Lab generator agent** | [`.github/agents/lab-generator.agent.md`](https://github.com/microsoft/FrontierWeekHack/blob/main/.github/agents/lab-generator.agent.md) | Encodes the entire 5-challenge curriculum + agent design patterns as a reusable Copilot agent. Our curriculum template should start here. |
...
| **Provisioning script** | [`factory/challenge-0-setup/deploy.sh`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-0-setup/deploy.sh) | Reference for Foundry resource + project + model + App Insights wiring via CLI/REST; note the graceful Entra-fallback + `.env` autogen patterns. |
| **`.env` template** | [`factory/challenge-0-setup/.env.template`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-0-setup/.env.template) | Canonical env-var contract (endpoint, project conn string, App Insights, tracing flags, workflow name). |
| **Agent SDK reference impl** | [`factory/challenge-1-build/agents.py`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-1-build/agents.py) | Clean `PromptAgentDefinition` + `FunctionTool` + function-call-loop reference. |
| **Tracing setup** | [`factory/challenge-2-monitor/monitor.py`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-2-monitor/monitor.py) | Minimal `AIProjectInstrumentor` + `configure_azure_monitor` pattern + the "set env before import" gotcha. |
| **Workflow-as-YAML** | [`factory/challenge-4-deploy/deploy.py`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-4-deploy/deploy.py) `create_workflow_agent()` | Portal-format workflow YAML (`InvokeAzureAgent`/`EndConversation`) authored from the SDK — the code↔portal bridge. |
| **Eval datasets** | [`*/challenge-3-evaluate/eval_portal.jsonl`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-3-evaluate/eval_portal.jsonl), [`*/challenge-4-deploy/evaluation_dataset.json`](https://github.com/microsoft/FrontierWeekHack/blob/main/factory/challenge-4-deploy/evaluation_dataset.json) | Ready `query`+`ground_truth` shape for LLM-as-judge; copy the schema, expand the row counts. |
| **Domain data files** | `*/challenge-1-build/{sensor_data,claims_data,call_data}.json` | Three worked examples of the "5 entities × 4 metrics × thresholds × status" data model (2 warning + 1 critical seeded for interesting results). |
...
| **MkDocs site config** | [`mkdocs.yml`](https://github.com/microsoft/FrontierWeekHack/blob/main/mkdocs.yml) + `deploy-pages.yml` | Drop-in docs-site scaffolding mirroring the challenge structure. |
