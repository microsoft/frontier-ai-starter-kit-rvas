...
strong because every activity maps to a *real role on a compliance team* (data analyst → risk
...
- A **single artifact evolves** across all 5 activities: a 3-agent sequential pipeline → 4-agent
...
- **"Right tool for the right job"** is the repeated design mantra (`README.md`,
  `activity-2/readme.md`): Azure AI Foundry agents for conversational/stateful work, MCP for
...
**Data substrate (Activity 0):** Azure Cosmos DB (`FinancialComplianceDB`) containers for
...
`activity-0/data/` (`customers.json`, `transactions.json`, `regulations.jsonl`,
`ml_predictions.json`) and is loaded via `activity-0/seed_data.sh`.
...
|---|-------------|---------------------|------------------------|------------|-----------|
| 0 | **Setup & Data Ingestion** (`activity-0/readme.md`) | Fork → Codespaces → deploy Azure infra (one-click ARM), retrieve keys, seed Cosmos + AI Search | ARM `azuredeploy.json`, APIM Basic v2, Cosmos DB, AI Search; `get-keys.sh` auto-writes `.env` | ★ Easy | 30 min |
| 1 | **Microsoft Agent Framework** (`activity-1/README.md`) | Create 3 specialized agents; learn Executors/Edges/Workflows/Events; build a **sequential** workflow; visualize in DevUI | `ChatAgent`, `AzureAIAgentClient`, `create_agent`, `@executor`, `WorkflowBuilder().set_start_executor().add_edge().build()`, `HostedFileSearchTool`/`azure_ai_search` tool, Pydantic data contracts, DevUI | ★★★ Core | 60 min |
| 2 | **Connect to Alert MCP Server** (`activity-2/readme.md`) | Expose a pre-built REST API as an **MCP server** via APIM (no code); build a 4th MCP-backed agent; evolve to **parallel fan-out** 4-agent workflow | APIM "Expose API as MCP Server (Preview)", `McpTool`, `RequiredMcpToolCall` + `SubmitToolApprovalAction` approval loop, subscription-key header auth, fan-out edges | ★★★ Core | 60 min |
| 3 | **Observability / Tracing** (`activity-3/readme.md`) | Add OpenTelemetry tracing; export to App Insights; write KQL; build a Monitor Workbook; trace individual transactions end-to-end | OTel GenAI semantic conventions, `TelemetryManager` (`telemetry.py`), 3-tier spans (app/workflow/executor), business metrics, KQL, Azure Workbook template, batch runner | ★★★★ Advanced | 60 min |
| 4 | **Fraud Alert Frontend** (`activity-4/README.md`) | Deploy an Angular dashboard on Container Apps; wire it to APIM; configure CORS; lock CORS to the frontend origin | Container Apps deploy (prebuilt image `ghcr.io/dsanchor/fraud-alert-management-front`), APIM CORS policy, secretref env vars | ★★ Easy-Med | 30 min |
...
**Core MAF primitives taught (Activity 1 README):** Executors (processing units),
Edges (message routing/branching/fan-in/fan-out), Workflows (the executor+edge graph),
Events (real-time observability signals).

**Pattern 1 — Sequential orchestration (Activity 1).** Graph-based, *not* a "Magentic"/manager
pattern. Three `@executor`-decorated async functions chained with explicit edges:
```python
# activity-1/workflow/sequential_workflow.py (L535-539)
WorkflowBuilder()
...
**Pattern 2 — Parallel fan-out (Activity 2).** Same builder, but the risk node fans out to *two*
downstream executors that run concurrently:
```python
# activity-2/agents/sequential_workflow_chal2.py (L815-822)
WorkflowBuilder()
  .set_start_executor(customer_data_executor)
  .add_edge(customer_data_executor, risk_analyzer_executor)
  .add_edge(risk_analyzer_executor, compliance_report_executor)
...
- *Knowledge tool:* Risk Analyzer attaches Azure AI Search as a native tool
  ...
  giving explainable, citation-bearing risk reasoning (`activity-1/agents/risk_analyser_agent.py`).
- *Action tool:* Fraud Alert agent attaches an MCP server as a tool with an explicit
  ...
  (`activity-2/agents/fraud_alert_foundry_agent.py`).
...
**Observability as a first-class pattern (Activity 3).** `TelemetryManager` wraps OTel with
...
(`activity-3/workbooks/azure-workbook-template.json`).

> **Note on red teaming / evals:** There is **no** dedicated evaluation harness or red-teaming
> activity in this repo. "Trust" here means *observability + auditability + explainability*, not
> safety evals. This is a gap worth noting (see §5).

---

## 4. Facilitation & Curriculum-Design Techniques Worth Borrowing

1. **One evolving artifact, five acts.** The deal is a single fraud pipeline that grows each
   activity (3→4 agents, +tracing, +UI). Each `Conclusion` ends with a "What's Next" teaser that
   literally previews the next activity's first paragraph — strong narrative momentum.
2. **Role-as-agent mapping.** Each agent embodies a real compliance-team role, making abstract
   ...
   scenario.
3. **Provide-the-API, teach-the-wiring.** Activity 2 explicitly says "we will *not* implement the
...
4. **Placeholder-driven code completion.** Activity 2's `mcp_tool = < PLACEHOLDER FOR MCP TOOL >`
...
5. **DevUI before tracing.** Activity 1 uses the visual DevUI (node graph: green=done,
   purple=running, black=pending) to build intuition *before* Activity 3 introduces rigorous OTel
...
6. **Explicit data contracts taught early.** Pydantic models as "workflow data contracts" are
   introduced in Activity 1 §4 — teaches type-safe agent I/O as a named concept, not an
   afterthought.
7. **"Why this option" decision tables.** Activity 3 has a comparison matrix (App Insights vs OTLP
...
9. **Per-activity duration + difficulty banner** at the top of every readme sets expectations.
...
- **No evaluation or red-teaming activity.** Despite the "trust" branding, there's no eval harness
  (groundedness, accuracy of risk scores), no adversarial/jailbreak testing, no AI Foundry
  Evaluations or red-team agent. *We should add a dedicated evals + safety activity* — it's the most
...
- **Security shortcuts called out but not taught.** Activity 0 explicitly uses key-based auth +
...
- **No Magentic / manager-planner pattern.** Only sequential + fan-out are taught. A stretch
  activity using a manager/orchestrator agent (dynamic planning) would showcase the more advanced
...
- **No automated grading / completion checks.** Validation is "compare your screenshot to ours."
  Our hack could ship a `validate.py` per activity (we already do this pattern in
...
- **Heavy hardcoded business logic in prompts** (country lists, thresholds) duplicated across
  activities — a config/source-of-truth file would reduce drift.
...
|-------|-----------|-----------------|
| One-click infra | `activity-0/infra/azuredeploy.json` | ARM template: Cosmos + AI Search + APIM Basic v2 + Container Apps + App Insights in one deploy |
| Key bootstrap script | `activity-0/get-keys.sh` (23KB) | Pattern for auto-generating `.env` from a deployed RG |
| Seed script + data | `activity-0/seed_data.sh`, `activity-0/data/*` | Reusable synthetic fraud dataset (customers/transactions/regulations/ml_predictions) |
| Sequential workflow reference | `activity-1/workflow/sequential_workflow.py` (+ `.ipynb`) | Canonical MAF `WorkflowBuilder` + `@executor` + Pydantic-contract example |
| Parallel fan-out workflow | `activity-2/agents/sequential_workflow_chal2.py` (L815-822) | Fan-out edge pattern from one node to two concurrent executors |
| MCP-tool agent w/ approval loop | `activity-2/agents/fraud_alert_foundry_agent.py` | `McpTool` + `RequiredMcpToolCall`/`SubmitToolApprovalAction` governed tool execution |
| AI-Search-as-tool agent | `activity-1/agents/risk_analyser_agent.py` | `create_agent(... tools=[{"type":"azure_ai_search"}], tool_resources=...)` |
| Telemetry manager | `activity-3/telemetry.py` (456 lines) | Reusable 3-tier OTel + App Insights wrapper with business metrics |
| KQL + Workbook | `activity-3/readme.md` (KQL blocks), `activity-3/workbooks/azure-workbook-template.json` | Ready-made fraud-KPI dashboard JSON + timechart/piechart KQL |
| Batch simulator | `activity-3/batch_run/batch_runner.py`, `multi_transaction_simulator.py` | Generate load for dashboard/observability demos |
| DevUI harness | `activity-1/devui/` (`devui_launcher.py`) | Directory-based agent/workflow discovery + web UI for demos |
| APIM→MCP how-to | `activity-2/readme.md` + https://learn.microsoft.com/azure/api-management/export-rest-mcp-server | Expose any REST/OpenAPI API as an MCP server with zero code |
...
pipeline that visibly grows each activity (3→4 agents, then tracing, then a UI), with each
conclusion teasing the next activity so momentum never resets. Equally worth stealing is the
...
missing despite the "trust" branding: a **dedicated evaluations + red-teaming activity** and
**structured (non-regex) inter-agent contracts**, plus per-activity automated validation scripts.
