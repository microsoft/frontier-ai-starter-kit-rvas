...
> Analysis only — no activity content was edited.
...
hands-on-keyboard **build** effort across all four activities is closer to **1.5–2 hr**. The labelled
clock is padded by **waiting** (App Insights ingestion lag, async hosted provisioning) and
**copy-paste** (two of four activities hand over every line of code inline). Almost nothing asks a
...
literal read of each `activities/*/README.md`. "Real-code" = the participant authors or completes
...
| Activity | Labelled time | # steps | Real-code vs conceptual/placeholder | Realistic **guided** time | Realistic **from-scratch** time | Guidance | Verdict |
|---|---|---|---|---|---|---|---|
...
| **Evaluation & Red Teaming** | 1.25 hr | 5 (Step 1–5) | **1 real-code** (S3 add *one* rule to `NorthfieldDomainEvaluator`) · rest run the provided `evaluate.py`, click the portal, or write a prose findings note | **65–75 min** | **2.5–3 hr** | **Heavy** — `evaluate.py` harness, both datasets, `--gate`, `--dry-run` all pre-written | **Clock is honest** (red-team analysis + portal waits are real), **but the build is one evaluator rule.** Richest *conceptual* activity; thinnest *authoring* activity. |
...
labelled **~2 hr total** for 5 activities and is *also* mostly copy-paste — so our per-activity
...
advanced activities have no time/difficulty banner in the participant README at all** (times live only
...
Every Advanced activity should ship **three rungs** off the same backbone, so it stops being pure
...
- **(c) Stretch goals** — genuinely open, no single right answer. Most activities already have a stub
...
strip to create rung (b), per activity:**

| Activity | Strip to create rung (b) | Hand them only (the contract) |
|---|---|---|
...
[FWH] ships **one 5-activity spine across three interchangeable domains** (factory / claims /
...
|---|---|---|---|
| 1 | **DevUI visual-first before tracing** | [ATA §4.5] | Let teams *watch* the agent run as a node graph (green/purple/black) before the OTel rigor of the Tracing activity — build intuition, then instrument. We jump straight to spans. |
| 2 | **"Why this order" narrative** | [FWH §4.2] | Add a 2-sentence rationale to each Advanced README tying the activity to a real stake ("a wrongful course hold blocks registration"). Our READMEs explain *what*, rarely *why this is next*. |
| 3 | **Run-individually then run-orchestrated** | [ATA §4.11] | Have teams smoke-test each piece (`python <file>.py`) before wiring — isolates failures. Critical for the Deploy activity's high failure surface. |
| 4 | **Per-activity duration + difficulty banner — consistently** | [FWH §2], [ATA §4.9] | **Two of four Advanced READMEs show no time/difficulty at all** (times live only in `PLAN-V2.md`). Put a standard `⏱ 75 min · ⭐⭐⭐` banner at the top of every activity so expectations are visible. |
...
and honest per activity; and **#5 (cleanup/cost-hygiene script + wrapup)** — a one-file-per-tier add
...
1. **Adopt the 3-rung ladder** (§2) as the standard shape for all four Advanced activities — it's the
   single biggest lever on "short/over-guided," and it adds no validators.
2. **Add the visible per-activity banner** (§4 #4) and **rung labels** so depth is opt-in and honest.
...
5. **Budget Deploy at 90 min**, not 60 — it's the one activity whose clock is optimistic, not padded.
