...
description: "Given a customer name and industry, research public sources and generate ~10 ranked AI-application ideas grounded in that company's real activities — each mapped to session tiers, tagged for effort, and wired to the Customer Outcome Canvas Pre-work fields so a participant can pick an idea and build immediately."
argument-hint: "Company name and industry are required. Optional: region/segment and known pain points."
---

## Context

Use this skill when a session participant has been asked to build with a customer scenario but
...
calibrated to the event window, maps each idea to the session's technology tiers, and then
pre-fills the Customer Outcome Canvas Pre-work fields for the top pick.

Use it **before** the session starts, ideally during the pre-work session or the first
30 minutes of day one. Once the team has chosen an idea, they carry the Canvas straight into
the Foundations activity.
...
**2b. Public statements of business activities**
...
- Top 5 activities the sector faces right now (regulatory shifts, margin pressure, talent gaps,
...
the session's event window (see Difficulty Calibration below).
...
| **Tier / tech mapping** | Which session tiers and activities this idea exercises (use exact names below) |
...
**Tier and activity names to use (use these exact labels):**
...
| Tag | What it means |
|---|---|
| `Starter` | Single-tier build (Foundations only). Grounded FAQ assistant, no actions. Completable in ~2 hours by a first-timer. Good first activity for a new team. |
| `Core` | Two-tier build: Foundations + one Advanced activity (typically Action Tools or Evaluation & Red Teaming). The sweet spot for most session teams. Grounded agent + one governed action or a scored eval suite. Completable in a full event day. |
...
|---|---|
| Not too trivial | A generic FAQ bot with no domain grounding is not a session deliverable. Every idea must use a real knowledge source specific to the customer's domain. |
...
Rank by: best fit for the company's stated activities, achievability in one event day, and
...
takes into their facilitator conversation.
...
carry it straight into the Foundations activity. Include placeholder text where the team must
...
| **Generic industry-blind ideas** | An idea that could apply to any company in any sector (e.g. "build a chatbot for employees") is not grounded. Every idea must reference a specific activity or opportunity from the research. |
...
| **Trivial demos** | A plain FAQ bot with no domain knowledge, no actions, and no evaluation is a tutorial output, not a session deliverable. Minimum bar: Foundations end-state (grounded answers with citations). |
...
**Bad industry/company fit rationale:** "Retail companies often have customer service activities."
...
- The Northfield University corpus (the session's default scenario) remains the safe fallback if
...
- After the team picks an idea, the next step is the [Customer Outcome Canvas](docs/customer-outcome.md)
  full discussion with their facilitator, then straight into the Foundations activity.
