...
# Coach Guide · Capstone — Northfield IQ, the Team

> **Coach-only.** This is the lowest-guidance challenge in the curriculum by design — there is
...
> ⚠️ Search-Before-Implement is a coach-enforced rule here. MAF is Oct-2025 and much of it is
...
up: green = done, purple = running, black = pending. Coaching cue: ask *"why is the synthesizer still
...
After DevUI, turn on the OTel GenAI tracing from the Tracing challenge and re-run one question.
...
which is the Deploy challenge's async variant). The criterion is a run that completes after the tab is
...
This is a long, open challenge. Don't let teams disappear for 2.5 hr. Set explicit reconvene checkpoints:
