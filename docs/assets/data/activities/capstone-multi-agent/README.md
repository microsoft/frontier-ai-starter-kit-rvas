...
>
> Prerequisites: the Foundations end-state (a deployed, grounded sample IQ assistant)
...
cited, governed answer. You orchestrate them with the Microsoft Agent Framework (MAF), using its
workflow primitives to compose agents into a code-first graph.

This activity is deliberately less guided. There is no starter file with `< PLACEHOLDER >` gaps to
...
6. Visualize the workflow state and events in DevUI, then instrument with the OTel tracing you already
...
workflow = (
    ...
    .add_fan_in_edges(                                    # waits for both branches
...
[Reference Library — Multi-agent orchestration](https://github.com/microsoft/frontier-ai-starter-kit-rvas/blob/main/docs/reference.html)
chapter walks you through, decision by decision. Build the sample organization version below first; it is the
...
- [Advanced · Action Tools](activity.html?id=advanced-action-tools) — the approval loop you reuse
- [Advanced · Tracing & Observability](activity.html?id=advanced-tracing-observability) — the OTel setup for Step 5
- [Advanced · Deploy as a Hosted Agent](activity.html?id=advanced-deploy-hosted-agent) — the hosted variant
- [Extra · Magentic Workflows](activity.html?id=extra-magentic-workflows) — the Magentic manager stretch
- [Extra · Hosted Long-Running](activity.html?id=extra-hosted-longrunning) — the background-run deploy variant
