...
> fictional *Northfield University*. Its [Foundations steps](activity.html?id=foundations) are worked
> examples you copy commands from; you don't need to build the Northfield version first.

> Before you start this chapter: complete [Define your outcome](activity.html?id=customer-outcome). You need your target
> user, business outcome, knowledge sources, and one safe scenario question written down. No idea
> yet? Run [Idea Forge](activity.html?id=idea-forge) first.
...
[scenario record](activity.html?id=customer-outcome):
...
scenario-specific yet. → [Foundations Step 1](activity.html?id=foundations#step-1--setup--provisioning-foundry--ai-search)
...
prompts and instruction. → [Foundations Step 2](activity.html?id=foundations#step-2--model-selection--the-playground)
...
instructions; test that your specific refusals actually hold. → [Foundations Step 3](activity.html?id=foundations#step-3--your-first-agent)
...
- Adapt it if your knowledge lives in a database or API, not files — note it here and deliver
  that retrieval as an action in [Action Tools](activity.html?id=customer-action-tools) instead.
...
2. Follow the indexing → knowledge base → attach-tool mechanics, substituting your files and your
   grounding instruction. → [Foundations Step 4](activity.html?id=foundations#step-4--knowledge-base-index--foundry-iq---foundations-end-state)
...
This is the prerequisite for every later chapter. Next: [Make your agent act](activity.html?id=customer-action-tools).
