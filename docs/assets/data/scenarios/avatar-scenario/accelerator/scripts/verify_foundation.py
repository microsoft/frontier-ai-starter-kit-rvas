#!/usr/bin/env python3
"""Module 2 checkpoint — prove the Foundry + Speech foundation is real and keyless.
...
Checks, in order:
  ...
  3. (live) Azure AI Search answers using Entra ID.
  4. (live) Both the chat and embedding deployments exist.
  5. (live) The Speech data plane answers using an Entra token (avatar/Voice Live keyless).

Run:
    python3 scenarios/avatar-onboarding/accelerator/scripts/verify_foundation.py

Offline/structure-only (no Azure calls, used by CI and the scenario validator):
...
REQUIRED_ENV = (
    ...
    "AZURE_EXPERIENCE_OUTPUT_CONTAINER_NAME",
)

FORBIDDEN_ENV_SUBSTRINGS = ("API_KEY", "ACCOUNT_KEY", "CONNECTION_STRING", "SAS_TOKEN", "SUBSCRIPTION_KEY")
...
def verify_live(env: dict[str, str], failures: list[str]) -> None:
    try:
        ...
        check(False, f"missing SDK dependency: {error}. Install with: pip install -r requirements.txt", failures)
        return
    ...
    # Speech data plane: request an Entra token for the Cognitive Services scope. A successful
    # token acquisition against a resource with a custom subdomain is what makes avatar / Voice
    # Live keyless. We list avatar batch jobs to confirm the data plane accepts the token.
    try:
        import urllib.request

        token = credential.get_token("https://cognitiveservices.azure.com/.default").token
        url = f"{env['AZURE_SPEECH_ENDPOINT']}/avatar/batchsyntheses?api-version=2024-08-01"
        request = urllib.request.Request(url, headers={"Authorization": f"Bearer {token}"})
        with urllib.request.urlopen(request, timeout=30) as response:  # noqa: S310 - fixed https host
            ok = response.status == 200
        check(ok, "Speech avatar data plane answered using an Entra token (keyless)", failures)
    except Exception as error:  # noqa: BLE001
        check(False, f"Speech data-plane keyless call failed: {error}", failures)
...
def main() -> int:
    ...
    print("== Module 2 checkpoint: Foundry + Speech foundation ==")
