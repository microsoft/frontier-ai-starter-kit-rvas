...
`source-manifest.json` is the source-of-truth metadata for the local simulation. Each document also
...
`../golden-questions.json` defines each local test case, expected citation IDs, role, pass condition,
and refusal reason. It covers routine policy, exceptions, freshness, confidential-source access,
prohibited data, and insufficient evidence. The simulation produces evidence without calling an AI service.
