...
- **Losing the response between requests.** The approval is a second HTTP call — the BFF must keep
  the response id and pending call around (an in-memory dict keyed by session id is fine here).
