...
- [`assets/sample-eval.jsonl`](assets/sample-eval.jsonl) — 36 grounded Q/A rows derived from the
...
2. Upload [`assets/sample-eval.jsonl`](assets/sample-eval.jsonl). Map `query` → query column and
...
**Tasks:**
1. In [`evaluate.py`](evaluate.py), study `sample organizationDomainEvaluator`. It returns a 1–5 score and
...
You get [`assets/sample-eval.jsonl`](assets/sample-eval.jsonl) and
...
3. Trace-to-eval correlation. After the Tracing activity, curate a *new* eval dataset from
