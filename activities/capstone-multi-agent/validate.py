...
"""Structural checkpoints for the Tier 3 Capstone · Northfield IQ, the Team (MAF).
    ...
    python validate.py --list                # which criteria are auto vs facilitator-judged
...
with a facilitator and are intentionally NOT asserted here.
...
# --- advisory (NOT required for exit 0; README marks these facilitator-confirmed) ---
...
def advise_reuse(scan: Scan, track: str) -> None:
    ...
    print("\n— advisory (facilitator-confirmed live, not gating) —")
    if track == "customer":
    ...
    if not (kb and approval):
        print("    note: reuse is graded LIVE with your facilitator — these are hints, not failures")
...
ADVISORY (printed, NOT gating — README marks these facilitator-confirmed):
...
def main() -> int:
    ...
    group.add_argument("--list", action="store_true",
                       help="List which criteria are auto-graded vs facilitator-judged.")
    parser.add_argument(
